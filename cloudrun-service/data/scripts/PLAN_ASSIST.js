// PLAN ASSIST • Inserção em lote (baseline)
void setTimeout(async () => {
  try {
    ...
    for (let i = 0; i < codigos.length; i++) {
      ...
      if (!evento) {
        alert("Campo EVENTO não encontrado. Ajuste PLAN_ASSIST.AMBOS.js");
        return;
      }
      evento.focus();
      evento.value = "";
      ...
      console.log("✔ PLAN ASSIST inserido:", code);
      await delay(1600);
    }

    console.log("🎉 PLAN ASSIST finalizado.");
  } catch (e) {
    console.error("❌ PLAN ASSIST erro:", e);
  }
}, 0);
