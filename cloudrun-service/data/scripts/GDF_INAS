/*@maskara{
  "mustUrlIncludes": ["gdf", "inas", "autorizador", "pagemain.aspx"],
  "detectAny": [
    "input[name='EVENTO']",
    "img#EVENTO_btn",
    "a[title*='Salvar / Novo']",
...
(() => {
  ...
  // Reinjeção => continua
  if (window.__HP_GDF_INAS_API__?.resume) {
    try { window.__HP_GDF_INAS_API__.resume("reinjected"); } catch {}
    return;
  }
  window.__HP_GDF_INAS_API__ = { resume: async () => {} };

  const payload = window.__HP_PAYLOAD__ || {};
  const scope = "GDF_INAS";
  ...
  // ✅ Ajustes rápidos
  // =========================
  const CODIGO_TABELA_DEFAULT = "22"; // <- TROQUE se o INAS usa outro
  const STORE_KEY = "hp_runner_state_gdf_inas_v1";
  ...
  async function ghostType(el, text, charDelay = 25) {
  ...
  function pageHasErrorHint() {
    ...
    return (
      ...
      t.includes("erro") && t.includes("campo")
    );
  }
  ...
  function pickBestRow(rows, codeDigitsOnly) {
    if (!rows.length) return null;
    const found = rows.find((a) => {
      const tr = a.closest("tr");
      const txt = (tr?.innerText || "").replace(/\s+/g, " ").trim();
      const digits = txt.replace(/\D/g, "");
      return codeDigitsOnly && digits.includes(codeDigitsOnly);
    });
    return found || rows[0];
  }
  ...
  async function tryPickFromPopupMain(codeDigitsOnly, timeoutMs = 25000) {
    ...
    while (Date.now() - t0 < timeoutMs) {
      ...
      if (pop && pop.document) {
        ...
        const picked = pickBestRow(rows, codeDigitsOnly);
        if (picked) {
  ...
  function tryPickFromThisDocPopup(codeDigitsOnly) {
    ...
    const picked = pickBestRow(rows, codeDigitsOnly);
    if (!picked) return { ok: false, reason: "no_rows" };
  ...
  async function stepOnce() {
    ...
    // POPUP DOC (quando o popup roda no mesmo doc/frame)
    if (IS_POPUP_DOC) {
    ...
    // 2) escolheu popup -> tabela -> limpar GRAU -> salvar/novo
    if (st.phase === "picked_popup" && st.lastCode) {
      ...
      if (ct) {
        await ghostType(ct, CODIGO_TABELA_DEFAULT, 15);
        log("✅ Código tabela preenchido:", CODIGO_TABELA_DEFAULT);
      } else {
  ...
  window.__HP_GDF_INAS_API__.resume = resume;
  ...
  log("🛡️ Runner + Watchdog (GDF INAS) ativos", { total: (getCodes() || []).length });
})();
